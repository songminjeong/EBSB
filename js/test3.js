...
function addElEvent(element, node_link) {
    element.addEventListener('click', function (evt) {
        let pre_id = cur_node; //클릭 전 아이디
        //cur_node = v1, v2, v3 ..

        var clickTime = new Date();
        var video_dom_t = document.querySelector('#' + pre_id + '_v');
        video_dom_t.muted = true;
        time = video_dom_t.currentTime;
        
        cur_node = element.id; //클릭아이디
        console.log("2"+cur_node)
...
function preload_seg(node_link) {
    ...
    for (let i in node_link[cur_node].link) { 
        ...
        if (node_link[cur_node].link[i].distance <7) {
